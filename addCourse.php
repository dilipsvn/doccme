 ...
 *	
 *	this class adds cme course
 *	whose details are filled in the
 *	form by registered cme provider
 *
 *	revised on 31/05/2007
 */
session_start();
 ...
 class Page
 {
	...
	var $SpecialityList;
 	...
 	function Page()
 	{
		...
		$this->emailList = new EmailList();
 		...
 		$this->courseTitle = new TextBox("CME Course Title", "courseTitle",$this->course->courseTitle, "200", "35");
 		$this->courseDesc = new TextArea("CME Course Description", "courseDesc", $this->course->courseDesc, "5", "35", "");
 		$this->specialityField = new TextBox("", "specialityField",$this->course->specialityField, "250", "35");
		...
		if($role == "CMEProvider")
		{
			...
			{
				$this->security->setLandingPage("addCourse.php");
				session_write_close();
 	...
 	function formLoad()
	{
		...
		if($this->PageAction=="SAVE")
		{
			...
			if($this->condt)
			{
				$this->error = $this->bal->addCMECourse($this->course);
	...
	{
		//$this->formLoad();
		include ("./html/html.AddCourse.php");
	}
	...
	function checkCourseTitle()
	{
		if($this->PageAction == "SAVE")
		{
			...
			else
			{
				...
				elseif(strlen($this->courseTitle->data) > 200)
				{
						return "Course title must contain less than 200 characters";
				}
				else
				{
						return "";
	...
	function checkSpecialityField()
	{
		if($this->PageAction == "SAVE")
		{
			if($this->specialityField->data != "")
			{
				...
				elseif(strlen($this->specialityField->data) > 250)
				{
						return "Speciality field must contain less than 250 characters";
				}
				else
				{
						return "";
...
$myPage = new Page();
$mPage = new Main("Add CME Course", $myPage);
$mPage->showPage();
