 ...
 */
 class QueryBuilder
 {
 	...
 	{
 		if($this->condition != "" && $this->fieldList != "" && $this->orderByCond == "")
 		{
 			$this->query = "SELECT ". $this->fieldList . "FROM " . $this->tableName . " WHERE " . $this->condition;
 		
 		}
 		elseif($this->condition != "" && $this->fieldList != ""  && $this->orderByCond !== "")
 		{
 			$this->query = "SELECT ". $this->fieldList . "FROM " . $this->tableName . " WHERE " . $this->condition . " ORDER BY " . $this->orderByCond;
 		}
 		elseif($this->condition == "" && $this->fieldList != "" && $this->orderByCond == "")
 		{
