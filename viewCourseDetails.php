...
class Page
{
	...
	function Page()
	{
		...
		/*if($role == "")
		{
			$this->security->setLandingPage("viewCourseDetails.php");
			session_write_close();
		...
		$id =  $_GET["id"];
		$result1 = "";
		$this->error = $this->bal->getCourseDetailsToView($result1, $id);
			...
			if($result1 != null)
			{
				while($course = mysql_fetch_assoc($result1))
				{
					...
					$this->course->courseDate->lastDateForApp = $this->lastDateForApp->data;
				 	...
				 	$this->status = new TextBox("Status", "status", $course["Status"], "50", "35");
...
$myPage = new Page();
$mPage = new Main("View Course Details", $myPage);
$mPage->showPage();
session_write_close();
?>
