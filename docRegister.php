 ...
 * 
 *	this class enables a Doctor
 *	to register in DocCME by filling 
 *	in the details
 *	after registration, Doctor can
...
class Page
{
	...
	function Page()
	{
		...
		$this->address1 = new TextBox("Address 1","address1",$this->doctor->address->address1,"250","25");
		$this->address2 = new TextBox("Address 2","address2",$this->doctor->address->address2,"250","25");
		...
		$this->bal->getComboSexList($this->SexList);
		...
		if($this->PageAction=="SAVE")
		{
			...
			if($this->condt)
			{
				...
				if(!$this->error->isError())
				{
					...
					{
						$mailSuccess = false;
						$mailErrorMessage = "Error in sending email to doctor : " . $this->firstName->data;
					}
					//send mail to admin
					...
					{
						$mailSuccess = false;
						$mailErrorMessage = "Error in sending email to admin";
					}
					session_write_close();
					header("location:./login.php?p=DOCTOR&msg=success");
					if(!$mailSuccess)
						echo $mailErrorMessage;
				}
			}
		}
	}
	...
	{
		//$this->formLoad();
		include ("./html/html.docRegister.php");
...
$mPage = new Main("Doctor Register",new Page());
$mPage->showPage();
session_write_close();
?>
